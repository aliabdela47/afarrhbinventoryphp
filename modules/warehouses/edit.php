...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (empty($errors)) {
        try {
            $oldData = $warehouse;
            $newData = compact('name', 'location', 'contactperson');
            
            Database::query(
                "UPDATE WAREHOUSES SET name = ?, location = ?, contactperson = ? WHERE warehouseid = ?",
                [$name, $location, $contactperson, $id]
            );
            
            auditLog('Update Warehouse', 'WAREHOUSES', $id, $oldData, $newData);
...
<div class="container-fluid">
    ...
    <div class="d-flex justify-content-between align-items-center mb-4">
        <div>
            <h1 class="h3 mb-0">
                <i class="bi bi-pencil"></i> <?php echo __('edit_warehouse'); ?>
            </h1>
